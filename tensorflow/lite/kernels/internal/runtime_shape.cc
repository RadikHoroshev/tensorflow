...
bool RuntimeShape::CheckedSizeRange(int start, int end, size_t& out) const {
  if (start < 0 || end < start || end > size_) return false;
  out = 1;
  const int32_t* dims_data = DimsData();
  for (int i = start; i < end; ++i) {
    if (!CheckedMul(out, dims_data[i], out)) return false;
  }
  return true;
}

bool RuntimeShape::CheckedSizeRange(int start, int end, int& out) const {
  size_t checked_out = 0;
  return CheckedSizeRange(start, end, checked_out) &&
         CheckedCast(checked_out, out);
}

bool RuntimeShape::CheckedSizeToDimension(int end, size_t& out) const {
  return CheckedSizeRange(0, end, out);
}
...
bool RuntimeShape::CheckedSizeFromDimension(int start, size_t& out) const {
  return CheckedSizeRange(start, size_, out);
}
...
bool RuntimeShape::CheckedFlatSize(size_t& flat_size) const {
  return CheckedSizeRange(0, size_, flat_size);
}
...
bool RuntimeShape::CheckedFlatSizeSkipDim(int skip_dim,
  ...
  size_t prefix = 0;
  if (!CheckedSizeRange(0, skip_dim, prefix)) return false;
  size_t suffix = 0;
  if (!CheckedSizeRange(skip_dim + 1, size_, suffix)) return false;
